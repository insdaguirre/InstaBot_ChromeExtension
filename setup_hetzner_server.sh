...
# Get latest ChromeDriver version compatible with installed Chrome
CHROME_VERSION=$(google-chrome --version | grep -oE "[0-9]+\.[0-9]+\.[0-9]+" | head -1)
echo "Detected Chrome version: $CHROME_VERSION"

# Download ChromeDriver using the new Chrome for Testing endpoints
LATEST_CHROMEDRIVER=$(curl -s "https://googlechromelabs.github.io/chrome-for-testing/LATEST_RELEASE_$CHROME_VERSION" 2>/dev/null || echo "")

if [ -z "$LATEST_CHROMEDRIVER" ]; then
    echo "⚠️ Specific version not found, using latest stable ChromeDriver..."
...
wget -O /tmp/chromedriver.zip "https://storage.googleapis.com/chrome-for-testing-public/$LATEST_CHROMEDRIVER/linux64/chromedriver-linux64.zip" 2>/dev/null || {
    echo "⚠️ Download failed, trying alternative method..."
    # Fallback to webdriver-manager during Python setup
    echo "Will install ChromeDriver via webdriver-manager instead"
    CHROMEDRIVER_FALLBACK=true
}

if [ "$CHROMEDRIVER_FALLBACK" != "true" ]; then
    sudo unzip -j /tmp/chromedriver.zip chromedriver-linux64/chromedriver -d /usr/local/bin/ 2>/dev/null || {
        echo "⚠️ Extraction failed, will use webdriver-manager fallback"
        CHROMEDRIVER_FALLBACK=true
    }
    if [ "$CHROMEDRIVER_FALLBACK" != "true" ]; then
...
echo "🖥️ Installing Xvfb for headless operation..."
...
# If ChromeDriver system installation failed, webdriver-manager will handle it
if [ "$CHROMEDRIVER_FALLBACK" = "true" ]; then
